export const XSpriteArray = [
  {
    name: "idle",
    pos: [
      { x: 323, y: 17, width: 30, height: 34 },
      { x: 357, y: 17, width: 30, height: 34 },
      { x: 391, y: 17, width: 30, height: 34 },
    ],
  },
  {
    name: "jump",
    pos: [
      { x: 202, y: 57, width: 24, height: 46 },
      { x: 230, y: 57, width: 17, height: 46 },
      { x: 252, y: 57, width: 19, height: 46 },
    ],
  },
  {
    name: "fall",
    pos: [
      { x: 274, y: 57, width: 23, height: 46 },
      { x: 299, y: 57, width: 27, height: 46 },
      { x: 331, y: 57, width: 24, height: 46 },
      // { x: 356, y: 57, width: 30, height: 46 },
    ],
  },
  {
    name: "walk",
    pos: [
      // { x: 106, y: 106, width: 30, height: 35 },
      { x: 137, y: 106, width: 20, height: 35 },
      { x: 158, y: 106, width: 23, height: 35 },
      { x: 181, y: 106, width: 32, height: 35 },
      { x: 213, y: 106, width: 34, height: 35 },
      { x: 247, y: 106, width: 26, height: 35 },
      { x: 276, y: 106, width: 22, height: 35 },
      { x: 298, y: 106, width: 25, height: 35 },
      { x: 326, y: 106, width: 30, height: 35 },
      { x: 357, y: 106, width: 34, height: 35 },
      { x: 391, y: 106, width: 29, height: 35 },
    ],
  },
  {
    name: "shoot",
    pos: [
      { x: 132, y: 66, width: 31, height: 34 },
      { x: 168, y: 66, width: 29, height: 34 },
    ],
  },
  {
    name: "jumpShoot",
    pos: [
      { x: 66, y: 143, width: 29, height: 46 },
      { x: 97, y: 143, width: 24, height: 46 },
      { x: 121, y: 143, width: 27, height: 46 },
      { x: 149, y: 143, width: 31, height: 46 },
      { x: 181, y: 143, width: 31, height: 46 },
      { x: 213, y: 143, width: 30, height: 46 },
    ],
  },
  {
    name: "dash",
    pos: [
      { x: 285, y: 158, width: 28, height: 31 },
      { x: 317, y: 163, width: 38, height: 26 },
    ],
  },
  {
    name: "dashShoot",
    pos: [
      { x: 361, y: 158, width: 38, height: 31 },
      { x: 404, y: 163, width: 49, height: 26 },
    ],
  },
  {
    name: "walkShoot",
    pos: [
      // { x: 61, y: 192, width: 35, height: 35 },
      { x: 97, y: 192, width: 29, height: 35 },
      { x: 126, y: 192, width: 32, height: 35 },
      { x: 158, y: 192, width: 35, height: 35 },
      { x: 194, y: 192, width: 38, height: 35 },
      { x: 233, y: 192, width: 34, height: 35 },
      { x: 267, y: 192, width: 31, height: 35 },
      { x: 298, y: 192, width: 33, height: 35 },
      { x: 331, y: 192, width: 35, height: 35 },
      { x: 367, y: 192, width: 37, height: 35 },
      { x: 404, y: 192, width: 35, height: 35 },
    ],
  },
  {
    name: "spawn",
    pos: [
      { x: 115, y: 3, width: 8, height: 48 },
      { x: 127, y: 3, width: 22, height: 48 },
      { x: 152, y: 3, width: 31, height: 48 },
      { x: 187, y: 3, width: 31, height: 48 },
      { x: 220, y: 3, width: 31, height: 48 },
      { x: 254, y: 3, width: 31, height: 48 },
      { x: 288, y: 3, width: 31, height: 48 },
    ],
  },
];
